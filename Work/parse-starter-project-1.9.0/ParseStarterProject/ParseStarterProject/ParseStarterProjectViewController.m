...
- (void)viewDidLoad {
    ...
    //testObject2[@"foo"] = @"newBar5";
    testObject2[@"date"] = [NSDate date];
    //testObject2[@"createdAt"] = [NSDate date];
    PFObject * object = [PFObject objectWithClassName:@"User"];
    [object addObject:@"aaa" forKey:@"arrayList"];
    //[testObject2 addObject:object forKey:@"user"];
    [testObject2 addObject:@"abc" forKey:@"user"];
    ...
    [testObject2 saveInBackgroundWithBlock:^(BOOL succeeded, NSError * _Nullable error) {
        if (succeeded) {
            ...
            NSLog(@"After save: the testObject2's globalID is:%@", testObject2.objectId);
            NSLog(@"After save: the testObject2's foo is:%@", testObject2[@"foo"]);
        }
    }];
