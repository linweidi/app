...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    //thumbnail
    if (photo) {
        ThumbnailImage * thumbnail = [ThumbnailImage thumbnailWithURL:photo.thumbnail inManagedObjectContext:self.managedObjectContext];
        
        thumbnail.name = photo.title;
        thumbnail.imageURL = photo.thumbnail;
        thumbnail.data = [NSData dataWithContentsOfURL:[NSURL URLWithString:photo.thumbnail ]];
        
        cell.imageView.image = [UIImage imageWithData:thumbnail.data];
        
        
    }
    
    NSLog(@"the photo title is: %@", photo.title);
    NSLog(@"the photo description is: %@", photo.subtitle);
    return cell;
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([detail isKindOfClass:[PhotoImageViewController class]]) {
        ...
        RecentPhoto * recent = [RecentPhoto createRecentPhotoWithDate:[NSDate date] inManagedObjectContext:self.managedObjectContext];
        recent.photo = self.photos[indexPath.row];
    }
}
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    if ([sender isKindOfClass:[UITableViewCell class]]) {
        ...
        if ([segue.destinationViewController isKindOfClass: [PhotoImageViewController class]]) {
            ...
            Photo *photo = [self.photos objectAtIndex:path.row];
            ...
            // add the photo into Recent Photo database
            RecentPhoto * recent = [RecentPhoto createRecentPhotoWithDate:[NSDate date] inManagedObjectContext:self.managedObjectContext];
            recent.photo = photo;
